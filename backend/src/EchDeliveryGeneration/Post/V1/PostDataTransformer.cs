...
public class PostDataTransformer : IPostDataTransformer
{
    ...
    private void AddContestDataToDelivery(ref Delivery delivery, EVoting.Config_6_0.ContestType contest)
    {
    ...
    private (List<EVoting.Config_6_0.Configuration>, List<EVoting.Print_1_3.VotingCardList> votingCardLists) Deserialize(PostDataTransformerContext context)
    {
        var config = new List<EVoting.Config_6_0.Configuration>();
        var votingCardLists = new List<EVoting.Print_1_3.VotingCardList>();

        config.Add(_eVotingXmlSerializer.DeserializeXml<EVoting.Config_6_0.Configuration>(context.PostConfigPath!));
        votingCardLists.Add(_eVotingXmlSerializer.DeserializeXml<EVoting.Print_1_3.VotingCardList>(context.PostPrintPath!));
